...
import { getGhostRunSync, saveGhostRunToIndexedDB } from './leaderboardStorage';
...
export function saveGhostRun(
  ...
  const lastKey = getLastKey(subMode, durationOrWords, testType);
  saveGhostRunToIndexedDB(lastKey, record).catch(() => {});
  ...
  if (!existingPb || wpm > existingPb.wpm) {
    const pbKey = getPbKey(subMode, durationOrWords, testType);
    saveGhostRunToIndexedDB(pbKey, record).catch(() => {});
  }
}
...
export function getStoredGhostRecord(
  ...
  return getGhostRunSync(key) as GhostReplayRecord | null;
}
