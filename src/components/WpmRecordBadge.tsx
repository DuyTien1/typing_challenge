...
/**
 * Hàm tìm kiếm và suy luận kỷ lục WPM chính xác kèm mốc thời gian & chế độ
 */
export function resolveBestWpmRecord(params: {
  ...
  // 1. Bản ghi trực tiếp đã có dữ liệu hợp lệ
  if (params.bestWpmRecord && params.bestWpmRecord.wpm > 0) {
    return {
      ...params.bestWpmRecord,
      modeName: params.bestWpmRecord.modeName || getFriendlyModeTitle(params.bestWpmRecord.mode),
    };
  }

  // 2. Trường hợp là AI Bot
  if (params.isBot) {
    return {
      wpm: params.bestWpm || 70,
      mode: 'bot_arena',
      modeName: 'Đấu Trường Luyện Tập AI',
      timestamp: Date.now() - 3600000,
    };
  }

  const targetWpm = params.bestWpm || 0;

  // 3. Tra cứu trong matchHistory nếu có
  if (params.matchHistory && params.matchHistory.length > 0) {
    const validMatches = params.matchHistory.filter((m) => m && m.wpm > 0);
    if (validMatches.length > 0) {
      // Ưu tiên trận trùng khớp đúng targetWpm
      const exactMatch = validMatches.find((m) => m.wpm === targetWpm);
      if (exactMatch) {
        return {
          wpm: exactMatch.wpm,
          mode: exactMatch.modeId,
          modeName: exactMatch.mode || getFriendlyModeTitle(exactMatch.modeId),
          timestamp: exactMatch.timestamp,
        };
      }
      // Nếu không khớp chính xác, lấy trận có WPM cao nhất
      const highestMatch = [...validMatches].sort((a, b) => b.wpm - a.wpm)[0];
      if (highestMatch && highestMatch.wpm > 0) {
        return {
          wpm: highestMatch.wpm,
          mode: highestMatch.modeId,
          modeName: highestMatch.mode || getFriendlyModeTitle(highestMatch.modeId),
  ...
  // 4. Tra cứu trên Bảng Vàng máy chủ (highScores)
  if (params.highScores) {
    let matchedScore: { modeId: string; record: HighScoreRecord } | null = null;
    const cleanUser = (params.username || '').trim().toLowerCase();

    // Tìm kiếm theo tên người chơi
    if (cleanUser) {
      for (const [modeId, rec] of Object.entries(params.highScores)) {
        if (rec && rec.username && rec.username.trim().toLowerCase() === cleanUser) {
          if (!matchedScore || rec.wpm > matchedScore.record.wpm) {
            matchedScore = { modeId, record: rec };
          }
        }
      }
    }

    // Nếu chưa tìm thấy và targetWpm > 0, đối chiếu theo điểm số WPM
    if (!matchedScore && targetWpm > 0) {
      for (const [modeId, rec] of Object.entries(params.highScores)) {
        if (rec && rec.wpm === targetWpm) {
          matchedScore = { modeId, record: rec };
          break;
        }
      }
    ...
    if (matchedScore) {
      return {
        wpm: matchedScore.record.wpm,
        mode: matchedScore.modeId,
        modeName: getFriendlyModeTitle(matchedScore.modeId),
        timestamp: matchedScore.record.timestamp,
      };
    }
  }

  // 5. Nếu là người chơi hiện tại, kiểm tra localStorage
  if (params.isMe && typeof window !== 'undefined') {
    try {
      const stored = localStorage.getItem('fasttyping_best_wpm_record');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && parsed.wpm > 0) {
          return {
            ...parsed,
            modeName: parsed.modeName || getFriendlyModeTitle(parsed.mode),
          };
        }
      }
    } catch {}
  ...
  // 6. Nếu có targetWpm > 0 nhưng chưa rõ lịch sử cụ thể
  if (targetWpm > 0) {
    return {
      wpm: targetWpm,
      mode: 'vi_dau',
...
export const WpmRecordBadge: React.FC<WpmRecordBadgeProps> = ({
  ...
  const displayValue = bestWpmDisplay || (hasRecord ? `${record.wpm} WPM` : isMe ? 'Chưa lập kỷ lục' : '0 WPM');

  // Title string dùng cho native browser tooltip fallback
  const nativeTitle = hasRecord
    ? `Kỷ lục: ${record.wpm} WPM | Chế độ: ${record.modeName} | Đạt lúc: ${timeInfo.full} (${timeInfo.relative})`
    : 'Chưa có kỷ lục thi đấu';
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-2 mb-2.5">
          <div className="flex items-center gap-1.5 text-xs font-black text-amber-400 uppercase tracking-wide">
            ...
            <span>Thông Tin Kỷ Lục WPM</span>
          </div>
          {hasRecord && (
        ...
        {hasRecord ? (
          <div className="space-y-2.5 text-xs">
            ...
            <div className="pt-2 border-t border-slate-800/80 flex items-center gap-1.5 text-[10px] text-slate-400">
              ...
              <span>Kỷ lục tự động cập nhật khi bạn đạt tốc độ mới cao hơn.</span>
            </div>
          </div>
        ) : (
          <div className="text-xs text-slate-300 space-y-1.5 py-1">
            <div className="flex items-center gap-1.5 text-amber-300 font-bold">
              ...
              <span>Chưa có kỷ lục thi đấu</span>
            </div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Hoàn thành một ván đấu để hệ thống tự động ghi nhận chính xác thời điểm và chế độ thi đấu đạt kỷ lục!
            </p>
          </div>
        )}
