...
import { RotateCcw, Home, Clock, Sparkles, Zap, Lock } from 'lucide-react';
...
export const GameOverModal: React.FC<GameOverModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      <div className="w-full max-w-3xl max-h-[92vh] flex flex-col bg-slate-900 border border-slate-700/80 rounded-3xl shadow-2xl relative overflow-hidden my-auto">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-7 space-y-5">
          ...
          {(() => {
            ...
            if (isPlayerSurrendered) {
              return (
                <div>
                  <div className="text-4xl sm:text-5xl mb-1.5">🏳️</div>
                  <h2 className="text-2xl sm:text-3xl font-black text-rose-400">
                    BẠN ĐÃ ĐẦU HÀNG (CHƯA HOÀN THÀNH)
                  </h2>
                  <p className="text-xs text-slate-400">
                    Ván đấu này không được tính là hoàn thành. Không ghi nhận kỷ lục và không tính vào thành tựu.
                  </p>
                </div>
              );
            }

            if (isBossMode || gameMode === 'san_boss') {
              return (
                <div>
        ...
        {!isLoggedIn ? (
          <div className="p-3.5 rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-amber-950/20 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-left">
            ...
            <button
              id="btn-gameover-cultivation-login"
              type="button"
              onClick={() => {
                soundFx.playKeyClick();
                if (onOpenAuthModal) onOpenAuthModal();
              }}
              className="w-full sm:w-auto px-3.5 py-1.5 rounded-xl text-xs font-bold bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 transition-colors flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
            >
              <Sparkles className="w-3.5 h-3.5" />
              Đăng nhập để Tu Tiên
            </button>
          </div>
        ) : (
          cultivationState && (
        ...
        {!isLoggedIn && (
          <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-3 text-left animate-fadeIn">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xs font-bold text-amber-300">Chế độ Khách: Chưa ghi nhận Bảng Vàng & Không nhận Tu Vi</div>
                <p className="text-[11px] text-slate-300">Đăng nhập tài khoản để lưu kỷ lục chính thức, tích lũy tu vi và mở khóa Linh Đài Tu Tiên!</p>
              </div>
            </div>
            <button
        ...
        <div className="p-3.5 sm:p-4 bg-slate-950/95 backdrop-blur-md border-t border-slate-800 shrink-0 z-20 shadow-[0_-12px_24px_-4px_rgba(0,0,0,0.6)]">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <button
              ...
              className="w-full h-11 sm:h-12 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-colors cursor-pointer group active:scale-98"
            >
              <Home className="w-4 h-4 text-slate-400 group-hover:text-white transition-colors" />
              <span>Trang Chủ</span>
              <kbd className="ml-1 px-1.5 py-0.5 rounded text-[10px] font-mono bg-slate-900 border border-slate-700 text-slate-400 font-normal">Esc</kbd>
            </button>
            ...
            <button
              ...
              className={`w-full h-11 sm:h-12 px-4 rounded-xl font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-md ${
                !isSolo && (countdown === 0 || hasTimedOut)
                  ...
                  : 'bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-black shadow-amber-500/20 active:scale-98 cursor-pointer'
              }`}
            >
              ...
              <span>
                {isSolo
                  ? 'Chơi Tiếp'
                  : countdown === 0 || hasTimedOut
                  ? 'Đã Hết Giờ Phòng'
                  : 'Trở Về Phòng (Chơi Tiếp)'}
              </span>
              {!isSolo && countdown > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-amber-950/20 text-amber-950 text-[10px] font-mono font-black">
                  {countdown}s
                </span>
              )}
              <kbd className={`ml-1 px-1.5 py-0.5 rounded text-[10px] font-mono border font-black ${
                !isSolo && (countdown === 0 || hasTimedOut)
