...
export const PlayerSimpleProfileModal: React.FC<PlayerSimpleProfileModalProps> = ({
  ...
  // Resolve Outplay record for this player
  ...
  // Stats fallback: CHỈ hiển thị kỷ lục từ chế độ Outplay Yourself
  const bestWpmDisplay = player.isBot
    ? (player.botTargetWpm ? `~${player.botTargetWpm} WPM` : '65 WPM')
    : outplayRecord && outplayRecord.wpm > 0
    ? `${outplayRecord.wpm} WPM`
    : isMe
    ? 'Chưa lập kỷ lục'
    : 'Chưa có kỷ lục';
    ...
    >
      <div className="w-full max-w-sm bg-slate-900 border border-slate-700/90 rounded-3xl p-5 shadow-2xl space-y-4 relative animate-scaleUp text-left">
        ...
        <div className="grid grid-cols-2 gap-2.5 pt-1">
          {/* Kỷ lục */}
          <WpmRecordBadge
            bestWpm={outplayRecord?.wpm || 0}
            bestWpmDisplay={bestWpmDisplay}
            bestWpmRecord={outplayRecord}
          ...
          <div className="p-3 rounded-2xl bg-slate-950/90 border border-purple-500/30 space-y-2">
            <div className="text-[10px] uppercase font-bold text-amber-400 flex items-center justify-between">
              ...
              <span className="text-[9px] font-mono text-slate-400 font-normal">
                {Math.min(player.showcaseAchievements.filter((id) => Boolean(getAchievementById(id))).length, 4)}/4 Đang Trưng Bày
              </span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
              {player.showcaseAchievements.filter((id) => Boolean(getAchievementById(id))).slice(0, 4).map((achId) => {
