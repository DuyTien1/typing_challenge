...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  // Esc key listener to quickly close admin modal
  useEffect(() => {
  ...
  }, [editableConfig, onUpdateConfig, onClose]);
  ...
  const togglePool = (
    ...
    setEditableConfig((prev) => {
      ...
      if (mode === 'ngauHung') {
        currentPools =
          prev.ngauHung.difficulties[diffKey]?.allowedPools ||
          defaultConfig.ngauHung.difficulties[diffKey]?.allowedPools ||
          ['vi_nodau', 'en', 'numbers'];
      } else if (mode === 'doanChu') {
        currentPools =
          prev.doanChu.difficulties[diffKey]?.allowedPools ||
          defaultConfig.doanChu.difficulties[diffKey]?.allowedPools ||
          ['vi_nodau'];
      } else if (mode === 'sanBoss') {
        currentPools =
          prev.sanBoss.difficulties[diffKey]?.allowedPools ||
          defaultConfig.sanBoss.difficulties[diffKey]?.allowedPools ||
          ['vi_nodau', 'en', 'numbers'];
      }
      ...
      <div className="relative w-full max-w-5xl h-[92vh] max-h-[900px] flex flex-col bg-slate-900 border-2 border-amber-500/50 rounded-3xl shadow-2xl overflow-hidden text-slate-200">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-slate-950 border-b border-slate-800 shrink-0">
          ...
          <button
            ...
            type="button"
            onClick={handleClose}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl hover:bg-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer"
            title="Đóng (Esc)"
          >
            <kbd className="hidden sm:inline text-[10px] font-mono font-semibold px-1 py-0.5 rounded bg-slate-950 border border-slate-700 text-slate-400">
              Esc
            </kbd>
            <X className="w-5 h-5" />
          </button>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-2.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-mono px-2.5 py-1 rounded-lg bg-orange-950/40 border border-orange-800/50 text-orange-300">
                          Đã chọn: {editableConfig.ngauHung.difficulties[selectedNgauHungDiff]?.allowedPools?.length ?? 3}/5 loại
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {POOL_OPTIONS.map((opt) => {
                        const currentPools = editableConfig.ngauHung.difficulties[selectedNgauHungDiff]?.allowedPools || ['vi_nodau', 'en', 'numbers'];
              ...
              {activeTab === 'doan_chu' && (
                <div className="space-y-5">
                  ...
                  <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-2.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-mono px-2.5 py-1 rounded-lg bg-purple-950/40 border border-purple-800/50 text-purple-300">
                          Đã chọn: {editableConfig.doanChu.difficulties[selectedDoanChuDiff]?.allowedPools?.length ?? 1}/5 loại
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {POOL_OPTIONS.map((opt) => {
                        const currentPools = editableConfig.doanChu.difficulties[selectedDoanChuDiff]?.allowedPools || ['vi_nodau'];
                        const isChecked = currentPools.includes(opt.id);
                        return (
                          <CustomCheckbox
                            key={opt.id}
              ...
              {activeTab === 'san_boss' && (
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-2.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-mono px-2.5 py-1 rounded-lg bg-amber-950/40 border border-amber-800/50 text-amber-300">
                          Đã chọn: {currentBossDiffConfig.allowedPools?.length ?? 3}/5 loại
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {POOL_OPTIONS.map((opt) => {
                        const currentPools = currentBossDiffConfig.allowedPools || ['vi_nodau', 'en', 'numbers'];
                        const isChecked = currentPools.includes(opt.id);
                        return (
                          <CustomCheckbox
                            key={opt.id}
