...
export const AchievementsSection: React.FC<AchievementsSectionProps> = ({
  ...
  const [showcase, setShowcase] = useState<string[]>(() => {
    ...
    return sanitizeShowcase(getShowcaseAchievements(userId || (isAdmin ? 'Admin' : undefined)));
  });
  ...
  React.useEffect(() => {
    if (!effectiveIsLoggedIn) {
      ...
      setShowcase(sanitizeShowcase(propShowcase));
    } else {
      setShowcase(sanitizeShowcase(getShowcaseAchievements(userId || (isAdmin ? 'Admin' : undefined))));
    }
  ...
  // Xử lý bật/tắt trang bị vào Showcase (Tối đa 4)
  const handleToggleShowcase = (achId: string) => {
    ...
    if (showcase.includes(achId)) {
      ...
      if (showcase.length < 4) {
        ...
        setFeedbackMsg(`Đã trưng bày thành công (${nextShowcase.length}/4)!`);
      } else {
        // Đã đủ 4, thay thế vị trí thứ 4
        nextShowcase = [...showcase.slice(0, 3), achId];
        setFeedbackMsg('Đã đổi vị trí thứ 4 trong đài trưng bày!');
      }
    }
  ...
  return (
    <div className="space-y-4 text-left">
      ...
      {/* 1. ĐÀI DANH VỌNG TRƯNG BÀY (SHOWCASE - TỐI ĐA 4 THÀNH TỰU) */}
      <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-purple-950/40 border border-purple-500/30 shadow-lg relative overflow-hidden">
            ...
            <div>
              <h4 className="text-xs font-black uppercase tracking-wider text-white flex items-center gap-1.5">
                ...
                <span className="text-[10px] px-2 py-0.2 rounded-full bg-purple-500/20 text-purple-300 border border-purple-400/40 font-mono">
                  {showcase.length}/4 Thành Tựu
                </span>
              </h4>
              <p className="text-[11px] text-slate-400">
                {effectiveIsLoggedIn 
                  ? 'Chọn tối đa 4 danh hiệu Tiên Hiệp đã lĩnh ngộ để hiển thị trên hồ sơ và phòng chờ'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {[0, 1, 2, 3].map((slotIdx) => {
            ...
            const slotTitle = slotIdx === 0 ? 'Thần Vị Chủ Đạo' : `Hộ Đạo Vị #${slotIdx}`;
                ...
                {isUnlocked ? (
                  ...
                  >
                    {isShowcased ? (
                      ...
                      <>
                        <Sparkles className="w-3 h-3 text-amber-400" /> Trưng Bày ({showcase.length}/4)
                      </>
                    )}
                  </button>
                ) : !isLoggedIn && (
