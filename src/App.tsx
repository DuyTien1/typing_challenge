...
import { resolveBestWpmRecord } from './components/WpmRecordBadge';
  ...
  ngauHung: {
    difficulties: {
      normal: {
        ...
        totalRounds: 15,
        allowedPools: ['vi_dau', 'vi_nodau', 'en', 'numbers'],
      },
      legendary: {
        name: 'Huyền Thoại',
        icon: '👑',
        ...
        totalRounds: 20,
        allowedPools: ['vi_dau', 'vi_nodau', 'en', 'numbers', 'fullsize'],
      },
    },
  },
  doanChu: {
    difficulties: {
      ...
      hard: {
        ...
        totalRounds: 12,
        showHint: true,
        allowedPools: ['vi_dau', 'vi_nodau', 'en'],
      },
      legendary: {
        name: 'Huyền Thoại',
        icon: '👑',
        ...
        totalRounds: 15,
        showHint: false,
        allowedPools: ['vi_dau', 'en', 'numbers'],
      },
    },
  },
  sanBoss: {
...
export default function App() {
  ...
  const recordCurrentMatch = useCallback((data: {
    ...
    // LOGIC HOÀN THÀNH TRẬN ĐỂ ĐƯỢC THƯỞNG TU VI:
    // Người chơi phải hoàn thành toàn bộ trận thi đấu mới được tính là hoàn thành và được thưởng tu vi.
    // Đầu hàng, out phòng sớm ở chế độ multiplayer và đầu hàng cùng reset trong trận ở chế độ solo
    // sẽ không được tính là hoàn thành trận đấu và KHÔNG được thưởng tu vi.
    ...
    // Cập nhật Tu Vi và nhiệm vụ hàng ngày KHI VÀ CHỈ KHI hoàn thành toàn bộ trận đấu
    ...
    // KIỂM TRA VÀ BẬT THÔNG BÁO THÀNH TỰU TIÊN HIỆP MỚI NGAY KHI VỪA KẾT THÚC TRẬN ĐẤU (CHỈ DÀNH CHO NGƯỜI ĐÃ ĐĂNG NHẬP)
    const userNow = currentUserRef.current;
    if (userNow) {
      try {
        const nextTotalGames = (totalGamesRef.current || 0) + 1;
        const nextBestWpm = Math.max(bestWpmRef.current || 0, data.wpm || 0);
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          sanBoss: {
            difficulties: {
              normal: { ...DEFAULT_CONFIG.sanBoss.difficulties.normal, ...(parsed.sanBoss?.difficulties?.normal || {}) },
              hard: { ...DEFAULT_CONFIG.sanBoss.difficulties.hard, ...(parsed.sanBoss?.difficulties?.hard || {}) },
              hell: { ...DEFAULT_CONFIG.sanBoss.difficulties.hell, ...(parsed.sanBoss?.difficulties?.hell || {}) },
            },
          },
          ngauHung: {
            difficulties: {
              normal: { ...DEFAULT_CONFIG.ngauHung.difficulties.normal, ...(parsed.ngauHung?.difficulties?.normal || {}) },
              legendary: { ...DEFAULT_CONFIG.ngauHung.difficulties.legendary, ...(parsed.ngauHung?.difficulties?.legendary || {}) },
            },
          },
          doanChu: {
            difficulties: {
              normal: { ...DEFAULT_CONFIG.doanChu.difficulties.normal, ...(parsed.doanChu?.difficulties?.normal || {}) },
              hard: { ...DEFAULT_CONFIG.doanChu.difficulties.hard, ...(parsed.doanChu?.difficulties?.hard || {}) },
              legendary: { ...DEFAULT_CONFIG.doanChu.difficulties.legendary, ...(parsed.doanChu?.difficulties?.legendary || {}) },
            },
          },
        };
      }
  ...
  // Tự động đồng bộ và khôi phục kỷ lục WPM chi tiết (thời điểm & chế độ) nếu có bestWpm
  useEffect(() => {
    if (bestWpm > 0 && (!bestWpmRecord || bestWpmRecord.wpm < bestWpm)) {
    ...
    <div className="min-h-screen bg-[#0f1117] text-slate-100 flex flex-col selection:bg-amber-400 selection:text-black">
      ...
      {isProfileOpen && (
        <ProfileModal
          ...
          showcaseAchievements={currentUser?.showcaseAchievements || getShowcaseAchievements()}
          onUpdateShowcaseAchievements={handleChangeShowcaseAchievements}
      ...
      {/* Cultivation / Linh Đài Tu Tiên Modal */}
      <CultivationModal
        ...
        userAvatar={avatar}
        userFrame={userFrame}
        onSelectFrame={handleChangeFrame}
        onUpdateState={(next) => {
      ...
      {/* Admin Online Users List & Details Modal */}
      {isOnlineUsersOpen && isAdmin && (
        <OnlineUsersModal
          ...
          onJoinRoom={async (roomId) => {
            await handleModalJoinExistingRoom(roomId, gameMode);
